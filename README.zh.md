...
- 颜色面板：支持背景色、文字色、字体、字号、HEX 输入和快捷色板
- 全局编辑模式：选中一个元素后自动选中所有同类型元素，批量修改样式
- 元素移动：开启移动模式后可拖动元素调整位置，支持磁吸对齐参考线
- 全局缩放：快捷键放大 / 缩小所有元素，效果可保存
- 可拖动面板：颜色面板可拖动，避免遮挡编辑区域
- 状态提示：显示当前文件是否已经修改
...
打开「全局编辑」开关后，选中一个元素会自动选中页面中所有同类型元素，可一次性批量修改背景色、文字色、字体和字号。

### 元素移动

打开「移动模式」开关后，可拖动元素调整位置。拖动时自动显示磁吸对齐参考线（边缘、中心、相邻元素）。移动效果会保存到输出文件中。

### 全局缩放

在编辑模式下，使用快捷键可对所有元素进行等比缩放，效果会保存到输出文件中。
...
- 颜色、字体、字号修改会写入 inline style
- 元素移动和全局缩放通过 CSS `transform` 实现，会写入 inline style
...
- 多页面幻灯片布局中，移动模式对隐藏页面元素的位置计算可能不准确
